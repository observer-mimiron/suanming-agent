...
func TestBaziCalc_ValidInput(t *testing.T) {
	...
	pillarsAny, ok := r["pillars"].([]map[string]any)
	if !ok {
		t.Fatal("pillars is not []map[string]any")
	}
	if len(pillarsAny) != 4 {
		t.Fatalf("expected 4 pillars, got %d", len(pillarsAny))
	}
	for i, p := range pillarsAny {
		if p["name"].(string) == "" {
...
func TestBaziCalcKnownCase(t *testing.T) {
	...
	pillarsAny := data["pillars"].([]map[string]any)
	if len(pillarsAny) != 4 {
		t.Errorf("pillars=%d, want 4", len(pillarsAny))
	}
	if data["dayGan"].(string) == "" {
		t.Error("dayGan empty")
	}

	for _, p := range pillarsAny {
		if shiShen, ok := p["shiShen"]; !ok || shiShen.(string) == "" {
