...
func (t *YongShenTool) Execute(_ context.Context, params map[string]any) (any, error) {
	...
	// stemWx/branchHidegan 引用 tables.go 包级表；generates 保留 local（印星方向，
	// 与 dayun.go 的食伤方向 generates 语义相反，不能共享）。
	...
	shiShenPower := computeShiShenPower(dayGan, dayWx, allGan, allZhi, stemWx, generates, branchHidegan)
