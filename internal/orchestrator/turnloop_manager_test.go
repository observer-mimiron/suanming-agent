...
func TestTurnLoopSessionManager_StopAll(t *testing.T) {
	...
	count := 0
	m.Range(func(_ string, _ *adk.TurnLoop[string, *schema.Message]) bool {
		count++
		return true
	})
	if count != 2 {
...
func TestTurnLoopSessionManager_PushEndToEnd(t *testing.T) {
	...
	if s := m.GetSink("session-push"); s != sink {
