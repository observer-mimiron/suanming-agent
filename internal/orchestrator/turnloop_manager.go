...
// （GenInput 合并 messages / PrepareAgent 调 preflight+prefill+BuildSupervisor /
// OnAgentEvents 桥接 agentEventBridge 到 sink）。
//
...
// GetSink 返回 sessionID 当前绑定的 sink。OnAgentEvents 通过它桥接事件。
func (m *TurnLoopSessionManager) GetSink(sessionID string) EventSink {
...
// ClearSink 清除 sink 映射（HTTP 响应结束时调，避免泄漏）。
func (m *TurnLoopSessionManager) ClearSink(sessionID string) {
...
func (m *TurnLoopSessionManager) getOrCreate(sessionID string) *adk.TurnLoop[string, *schema.Message] {
	...
	getSink := func() EventSink { return m.GetSink(sessionID) }
...
// Range 遍历所有 TurnLoop 实例（测试/调试用）。
func (m *TurnLoopSessionManager) Range(fn func(sessionID string, loop *adk.TurnLoop[string, *schema.Message]) bool) {
	m.loops.Range(func(key, value any) bool {
		return fn(key.(string), value.(*adk.TurnLoop[string, *schema.Message]))
	})
}
