...
**注意**：项目中 "Supervisor" 一词容易混淆，它既可指 `RouteAdvisor`（路由决策层，Go ADK RouteEngine），也可指 `Supervisor Agent`（执行层的 LLM 调度 Agent）。本文以 "RouteAdvisor" 指路由层，"Supervisor Agent" 指执行层。
...
### AgentAsTool · Agent 即工具

**是什么**：Eino ADK 机制，将一个 Agent 包装为 Tool，供另一个 Agent 调用。本项目中 Supervisor Agent 通过 AgentAsTool 调用 Specialist Agent。

**行为特点**：Supervisor 调用哪个 Specialist、调用几次、何时停止，全部由 LLM 决定。约束手段是 supervisor instruction 中的自然语言规则，属于隐式控制模型。

**已知问题**：prompt 脆弱（模型升级/temperature 波动可能导致行为偏移），agentEventBridge 需要大量适配代码（specialistDone 标记、`_specialist` 后缀匹配、流式/非流式双路径）。

**演进方向**：Eino Graph 编排替代 AgentAsTool，把隐式调度变成 Go 侧确定性控制流。
...
- `specialistDone` 标记：防止 Specialist 响应被重复加入
...
**是什么**：确定性工具，排四柱命盘（含晚子时校正、太阳时校正、神煞）。由 prefill 阶段执行，不挂到 Specialist Agent。

**关联**：底层引擎是 [lunar-go](https://github.com/nicholasciang/lunar-go)（开源成熟方案，非自研）。
...
**是什么**：Eino 框架的 Agent 开发工具包。提供 ChatModelAgent（ReAct Agent 实现）、AsyncIterator、AgentAsTool 等。

**在项目中的角色**：RouteAdvisor 用 ChatModelAgent 做结构化路由，Supervisor Agent 用 ChatModelAgent + AgentAsTool 做执行调度。

---

### ChatModelAgent

**是什么**：Eino ADK 的 Agent 实现，支持 tool calling 循环。项目中的 Supervisor Agent 和 Specialist Agent 都是 ChatModelAgent 实例。
...
**是什么**：支持 tool calling 的 ChatModel 接口。Supervisor Agent 使用它来决定调用哪个 Specialist。

---

### lunar-go

**是什么**：开源八字排盘库。提供公历→农历转换、天干地支、节气排盘等功能。项目八字引擎的核心依赖。
...
**是什么**：用 Eino 的 compose.Graph 替代 AgentAsTool，把当前的隐式调度变成显式 Go 侧控制流。当前推迟，等 runtime 需要更深分支/并行/中断恢复时再做。

**目标拓扑**：`[Prefill] → [Domain Router] → [Specialist*] → [Bridge] → [Done]`
...
两者本质上是 AgentAsTool 隐式协议的补丁，Graph 编排后预计会简化或移除。
...
flowchart TD
    ...
    PF2 --> SA["Supervisor Agent<br/>LLM 调度层"]
    SA -->|AgentAsTool| SP["Specialist Agent<br/>领域专家 x3"]
    SP --> TK["knowledge_catalog<br/>knowledge_search"]
    SP --> BR["agentEventBridge"]
...
| 预填充（确定性排盘） | Prefill |
| 执行器 | Executor |
| 调度 Agent | Supervisor Agent |
| 领域专家 Agent | Specialist Agent |
| Agent 即工具 | AgentAsTool |
