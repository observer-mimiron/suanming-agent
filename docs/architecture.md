...
本命、全程运路与当前阶段是三个只读下游边界。静态层保留主轴、强弱、调候、格局取用及其依据；格局另以“主格、成局路线、子平定性”三个字段展示，不再维护九维或九级层次合同；最终强弱视角下方附“喜神、用神、忌神”确定性状态，缺失时显示待定，不由模型补算。`lifetime_dayun_judgment` 必须覆盖全部已计算大运，并只输出其对本命结构的补、助、损、破；`dynamic_judgment` 继续只裁断当前大运的 `current_period_realization`（修复、助力、维持、扰动、压制）和流年走势。后两层不得改写本命，当前层不得改写全程逐运或总评。最终报告先以本命总览收束主轴、格局定性与限制，再依次展开静态视角、全程运路和当前应期，避免重复同一岁运结论。
