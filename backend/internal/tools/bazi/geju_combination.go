...
func detectGejuCombination(dayGan, dayWx, strength string, allGan, allZhi []string,
	...
	godSources := func(god string) []string {
		sourcesInfo := []tenGodInfo{}
		for _, info := range allGods {
			if info.god == god {
				sourcesInfo = append(sourcesInfo, info)
			}
		}
		sort.SliceStable(sourcesInfo, func(i, j int) bool {
			if sourcesInfo[i].sourceWeight() != sourcesInfo[j].sourceWeight() {
				return sourcesInfo[i].sourceWeight() > sourcesInfo[j].sourceWeight()
			}
			if sourcesInfo[i].distanceToDayPillar() != sourcesInfo[j].distanceToDayPillar() {
				return sourcesInfo[i].distanceToDayPillar() < sourcesInfo[j].distanceToDayPillar()
			}
			return sourcesInfo[i].pillarIdx < sourcesInfo[j].pillarIdx
		})
		sources := make([]string, 0, len(sourcesInfo))
		for _, info := range sourcesInfo {
			sources = append(sources, info.displaySource())
		}
		return sources
	}
