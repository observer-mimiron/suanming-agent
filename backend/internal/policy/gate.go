...
func Apply(decision schemas.SupervisorDecision, st *state.SessionState) ApprovedRoute {
	route := ApprovedRoute{
		...
		Gate: contracts.GateContract{
			Admitted:            true,
			ProfileRequirement:  decision.PolicyHints.ProfileRequirement,
			ReuseSessionProfile: decision.PolicyHints.CanReuseSessionProfile,
			ReuseCachedResult:   decision.PolicyHints.CanReuseCachedResult,
			ExecutionMode:       "execute",
			GuidancePolicy:      "bypass",
...
func applyPhase1Allowlist(route *ApprovedRoute) {
	if !phase1Allowlist[route.PrimaryDomain] {
		...
		route.Gate.Reason = "phase1_allowlist"
...
func applyConfidenceClarification(route *ApprovedRoute, confidence float64) {
	if confidence < confidenceThreshold && !route.NeedsClarification {
		...
		route.Gate.Reason = "low_confidence"
...
func applyQimenRouting(route *ApprovedRoute) {
	if route.PrimaryDomain == "qimen" && !wantsQimenPrimary(*route) {
		...
		route.Gate.Reason = "qimen_downgraded"
		route.Gate.ExecutionMode = "collect"
	}
	route.Gate.AllowedDomains = append([]string{route.PrimaryDomain}, route.SecondaryDomains...)
}
