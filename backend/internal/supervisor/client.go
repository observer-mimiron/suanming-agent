...
// WithCheapGateReporter 注入 cheap gate 命中样本记录器。
func WithCheapGateReporter(reporter *observability.CheapGateReporter) ClientOption {
	return func(c *Client) {
		c.reporter = reporter
	}
}
...
type Client struct {
	...
	reporter    *observability.CheapGateReporter
}
