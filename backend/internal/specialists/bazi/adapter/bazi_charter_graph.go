// Package adapter 负责八字 Graph 的运行时能力适配。
//
...
import (
	...
	bazipresentation "github.com/observer-mimiron/suanming-agent/internal/specialists/bazi/presentation"
...
func collectBaziSoftAuditWarnings(state baziCharterState) []string {
	...
	if len(unknownFactRefs) > 0 {
		warnings = append(warnings, "assertions use unknown fact-ref aliases: "+strings.Join(bazidomain.UniqueStrings(unknownFactRefs), ", "))
	}
	if len(unknownClaimRefs) > 0 {
		warnings = append(warnings, "assertions use unknown claim refs: "+strings.Join(bazidomain.UniqueStrings(unknownClaimRefs), ", "))
	}
...
func (e *Executor) runFinalWriter(ctx context.Context, view *specialists.SessionView, chartState baziCharterState, question string) (string, error) {
	_ = view
	output := bazipresentation.RenderFinalReplyForState(chartState.AnalysisPlan, chartState, question)
	if err := bazipresentation.ValidateFinalReplyForState(chartState.AnalysisPlan, chartState, output); err != nil {
		tracing.SetTraceAttributes(ctx, map[string]any{
