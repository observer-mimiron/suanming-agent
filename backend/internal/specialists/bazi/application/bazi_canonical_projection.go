...
// mergeCitations 合并引用并按标识去重，避免投影层重复展示同一来源。
func mergeCitations(base []baziCitation, adds ...baziCitation) []baziCitation {
	out := append([]baziCitation(nil), base...)
	for _, add := range adds {
		duplicate := false
		for _, item := range out {
			if item.Classic == add.Classic && strings.Join(item.Quotes, "\n") == strings.Join(add.Quotes, "\n") {
				duplicate = true
				break
			}
		}
...
// ProjectCanonicalStaticSynthesis projects accepted canonical static fields.
func ProjectCanonicalStaticSynthesis(state bazidomain.CharterState, canonical bazidomain.CanonicalSynthesis) bazidomain.StaticSynthesis {
	return projectCanonicalStaticSynthesis(state, canonical)
}
...
func minorOutcomeSafeText(text, fallback string) string {
	...
	if domain, _ := bazidomain.FirstUnauthorizedMinorOutcomeSignal(text); domain != "" {
		return fallback
	}
	return text
}
...
func minorOutcomeSafeList(items, fallback []string) []string {
	...
	for _, item := range items {
		...
		if domain, _ := bazidomain.FirstUnauthorizedMinorOutcomeSignal(item); domain != "" {
			continue
		}
		out = append(out, item)
	}
...
// ProjectCanonicalDynamicSynthesis projects accepted canonical dynamic fields.
func ProjectCanonicalDynamicSynthesis(state bazidomain.CharterState, canonical bazidomain.CanonicalSynthesis, static bazidomain.StaticSynthesis) bazidomain.DynamicSynthesis {
...
func stringsToFactRefs(items []string) []baziFactRef {
	...
	for _, item := range bazidomain.NonEmptyStrings(items) {
...
func canonicalDayunEvidence(period map[string]any) []string {
	...
	if len(facts) > 0 {
		return bazidomain.UniqueStrings(facts)
	}
...
func mergeCanonicalDayunPath(facts []string, judgments []baziDayunJudgment) []string {
	if len(facts) == 0 {
		return bazidomain.RenderDayunJudgmentLines(judgments)
	}
...
func canonicalListOrDefault(items, fallback []string) []string {
	items = bazidomain.NonEmptyStrings(items)
	if len(items) > 0 {
		return items
	}
	return bazidomain.NonEmptyStrings(fallback)
}

// CanonicalListOrDefault returns non-empty items or the deterministic fallback.
func CanonicalListOrDefault(items, fallback []string) []string {
	return canonicalListOrDefault(items, fallback)
}
...
func canonicalFailureFactsOnly(state baziCharterState, err error, auditCode, fallback string) (baziStaticSynthesis, baziDynamicSynthesis) {
	...
	dynamic := bazidomain.BuildFactsOnlyDynamicSynthesis(state.Input, static, reason)
...
// CanonicalFailureFactsOnly projects the deterministic fallback after a rejected candidate.
func CanonicalFailureFactsOnly(state bazidomain.CharterState, err error, auditCode, fallback string) (bazidomain.StaticSynthesis, bazidomain.DynamicSynthesis) {
	return canonicalFailureFactsOnly(state, err, auditCode, fallback)
}
...
// CanonicalDynamicFailureFactsOnly keeps accepted static output and dynamic facts.
func CanonicalDynamicFailureFactsOnly(state bazidomain.CharterState, static bazidomain.StaticSynthesis, err error) bazidomain.DynamicSynthesis {
	return canonicalDynamicFailureFactsOnly(state, static, err)
}
