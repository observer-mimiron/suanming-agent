// package domain 包含八字合同 finding 的 runtime 投影辅助。
//
...
func baziContractAuditError(stage string, finding baziContractAuditFinding) error {
	field := firstNonEmpty(finding.Field, strings.TrimSpace(stage))
	message := firstNonEmpty(finding.Reason, finding.Code, "independent synthesis contract audit failed")
