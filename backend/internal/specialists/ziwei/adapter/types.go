...
// ZiWeiStar 是 adapter 对 domain 星曜值对象的兼容别名；旧 map 投影仍由本文件负责。
type ZiWeiStar = ziweidomain.ZiWeiStar

// StarPayload 是 adapter 输出星曜的 JSON DTO，维持既有工具 map payload 字段名和空字段省略规则。
...
func (c *ZiWeiChart) ToMap() map[string]any {
	...
	for i, p := range c.Palaces {
		palaces[i] = map[string]any{
			...
			"major_stars":        starPayloads(p.MajorStars),
			"minor_stars":        starPayloads(p.MinorStars),
			"adjective_stars":    starPayloads(p.AdjectiveStars),
...
// starPayloads 将领域星曜投影为既有工具输出 DTO，避免序列化合同反向进入 domain。
func starPayloads(stars []ZiWeiStar) []StarPayload {
	if stars == nil {
		return nil
	}
	payloads := make([]StarPayload, len(stars))
	for i, star := range stars {
		payloads[i] = StarPayload{
			Name:       star.Name,
			Type:       star.Type,
