...
func collectUserMeta(phase *ProcessPhaseDigest, span TraceSpan) {
	...
	if v, ok := span.Attributes["decision_source"]; ok {
		phase.Meta["decision_source"] = v
	}
	if v, ok := span.Attributes["reuse_reason"]; ok {
		phase.Meta["reuse_reason"] = v
	}
	switch span.Kind {
	case KindRetriever:
...
func buildPhaseSummary(phase ProcessPhaseDigest) string {
	switch phase.Key {
	case "route":
		if source, ok := phase.Meta["decision_source"].(string); ok && source == "cheap_followup_reuse" {
			return "已复用上一轮执行合同，跳过完整路由模型，直接进入追问主链。"
		}
...
func buildRuntimeMeta(t *TurnTrace) map[string]any {
	...
	if snapshot.Gate.ExecutionMode != "" {
		meta["execution_mode"] = snapshot.Gate.ExecutionMode
	}
	if snapshot.Gate.Reason != "" {
		meta["gate_reason"] = snapshot.Gate.Reason
	}
	if snapshot.Gate.FollowupPolicy != "" {
		meta["followup_policy"] = snapshot.Gate.FollowupPolicy
	}
