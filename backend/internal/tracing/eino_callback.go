	...
	return einoutils.NewHandlerHelper().
		ChatModel(&einoutils.ModelCallbackHandler{
			OnStart: func(ctx context.Context, info *einocallbacks.RunInfo, input *einomodel.CallbackInput) context.Context {
				...
				if input != nil && len(input.Messages) > 0 {
					setObservationAttribute(span, "input.value", serializeLLMMessagePreview(input.Messages))
			...
			OnEnd: func(ctx context.Context, _ *einocallbacks.RunInfo, output *einomodel.CallbackOutput) context.Context {
				...
				finishEinoCallbackSpan(ctx, nil, usage, output)
				return ctx
			},
		...
		Retriever(&einoutils.RetrieverCallbackHandler{
			OnStart: func(ctx context.Context, info *einocallbacks.RunInfo, input *einoretriever.CallbackInput) context.Context {
				...
				if input != nil {
					if input.Query != "" {
						setObservationAttribute(span, "input.value", input.Query)
		...
		OnStart: func(ctx context.Context, info *einocallbacks.RunInfo, input *einotool.CallbackInput) context.Context {
			...
			name = normalizeToolSpanName(name, input)
			span := SpanFromContext(ctx, name, KindTool)
			if input != nil && input.ArgumentsInJSON != "" {
				setObservationAttribute(span, "input.value", input.ArgumentsInJSON)
		...
		OnEnd: func(ctx context.Context, _ *einocallbacks.RunInfo, output *einotool.CallbackOutput) context.Context {
			...
			if span != nil {
				if output != nil && output.Response != "" {
					setObservationAttribute(span, "output.value", output.Response)
...
func AppendCurrentSpanAttribute(ctx context.Context, key string, value any) {
	...
	setObservationAttribute(span, key, value)
}

func normalizeToolSpanName(name string, input *einotool.CallbackInput) string {
	if name != "output" || input == nil {
		return name
	}
	args := input.ArgumentsInJSON
	if strings.Contains(args, "\"primary_domain\"") || strings.Contains(args, "\"conversation_intent\"") || strings.Contains(args, "\"task_intent\"") {
		return "supervisor_output"
	}
	return name
}

func finishEinoCallbackSpan(ctx context.Context, err error, usage *einomodel.TokenUsage, output *einomodel.CallbackOutput) {
	span := einoSpanFromContext(ctx)
	if span == nil {
		return
	}
	...
	if output != nil && output.Message != nil && strings.TrimSpace(output.Message.Content) != "" {
		setObservationAttribute(span, "output.value", output.Message.Content)
	}
	if err != nil {
		span.RecordError(err)
		span.SetStatus("error")
...
func finishEinoRetrieverSpan(ctx context.Context, output *einoretriever.CallbackOutput) {
	...
	if output != nil {
		if output.Docs != nil {
			...
			setObservationAttribute(span, "output.value", summarizeRetrieverDocs(output.Docs))
		}
		if output.Extra != nil {
...
func summarizeRetrieverDocs(docs []*schema.Document) string {
	parts := make([]string, 0, len(docs))
	for i, doc := range docs {
		if doc == nil {
			continue
		}
		content := redactTracePreview(strings.TrimSpace(doc.Content))
		if r := []rune(content); len(r) > 120 {
			content = string(r[:120]) + "...(truncated)"
		}
		if content == "" {
			continue
		}
		parts = append(parts, fmt.Sprintf("[%d] %s", i, content))
...
func setObservationAttribute(span Span, key string, value any) {
	if span == nil || key == "" {
		return
	}
