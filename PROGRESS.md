...
- **Runtime 整洁化治理 G0-G4：** G0 已完成 owner、状态和依赖基线；G1 已将 Graph 单轮状态、请求 context 携带器、Manager 的计划/会话投影/最终合成，以及事件桥接的 ADK 消费、chart 投影、输出标记投影拆为同 package 文件；G2 已用合同测试确认 Graph state、SessionState、Manager plan 和 specialist SessionView 的边界；G3 已对照 Coze Builder → Runner → Callback → Application Event 完成生命周期审查；G4 判定不适用，未复制 Coze checkpoint/stream 包装。未新增顶层 Orchestrator、Planner、ExecutionState、消息总线或 durable checkpoint，行为合同保持不变。台账见 [`docs/runtime-owner-ledger.md`](docs/runtime-owner-ledger.md)。
- **本轮最小在线评测：** Docker daemon 恢复后启动了 Langfuse 3.225.2，知识库已运行；`make regression` 实际启动临时后端 `127.0.0.1:18080` 并执行 `runtime-smoke-v2`。结果 `passed=2`、`failed=0`、`pass_rate=1.0`，报告为 `/tmp/suanming-agent/runtime-smoke-report.json`。首轮八字 trace 为 `d2248467b433194e9818d428a7405a25`，追问复用 trace 为 `5c33c2c4d5be3c01e0233c3ea2af891d`；两条均完成 SSE `done`，首轮 `bazi.final.audit_result=clean`，观察到 `preflight`、`sse_emit` 和 `contract_gate`。临时后端已由评测脚本清理，Langfuse 与知识库仍保持运行。
- **当前任务：** 八字业务收拢已完成本轮执行边界迁移。Graph 节点、模型/检索调用和 schema 位于 `specialists/bazi/adapter`，纯合同与事实位于 `domain`，用例投影位于 `application`，最终文本位于 `presentation`；runtime 不再持有八字 Graph/模型/证据实现。静态模型保留四个核心 claim、`axis_status`，以及“主格、成局路线、子平定性”三个格局字段；九维/九级层次合同已删除。最终报告去掉统一“结论：”前缀，喜神、用神、忌神拆为三行。`yong_shen/xi_shen/ji_shen` 由确定性工具按强弱与五行生克返回基础候选，不再固定为空数组；调候投影仅在模型无 verdict 时回退事实文案，已有检索裁断不再被覆盖。局部与全量 backend 测试、server build 均通过。
...
- **知识库与动态终态修复：** 知识库检索只补充古籍参照和已检索材料一致性，不再作为静态主轴或暗合路线的前置条件。2026-08-17 真实回放 `trc_cc00c1a360c7` 为 `status=ok`；首步大运尚未开始时，`bazi.loop_step=5`、`bazi.max_run_steps=24`，终态准确记录为 `current_period_unavailable`，不再误报 `graph_step_limit_degraded`。
...
- Runtime Clean Governance G0：已完成。通过 CodeGraph 和源码职责注释冻结 owner、状态生命周期、依赖方向、Coze 对照入口及事件终态不变量；`go list ./backend/...`、`go test ./backend/... -count=1`、`go build ./backend/cmd/server/` 均通过。产物为 [`docs/runtime-owner-ledger.md`](docs/runtime-owner-ledger.md)，未修改生产代码。
- Runtime Clean Governance G1-1：已完成。在 `internal/runtime` 同一 package 内将原 `orchestration_state.go` 按职责拆为 `orchestration_graph_state.go`（Graph 单轮状态）和 `orchestration_context.go`（请求 context 携带器）；未改变类型、函数签名、Graph 注册、拓扑、SSE、trace、错误出口或状态生命周期。runtime focused test、全量 backend test、server build 和 `git diff --check` 均通过。
- Runtime Clean Governance G1-2：已完成。在 `internal/runtime` 同一 package 内将 Manager 拆为 `manager.go`（owner/依赖）、`manager_plan.go`（路由修正与 ExecutionPlan）、`manager_context.go`（SessionState 最小上下文投影与 TurnContext）和 `manager_compose.go`（跨 specialist 最终合成）；共享 `firstNonEmpty` 归入 `value_helpers.go`。未改变公开/私有函数签名、Graph 拓扑、预算、SSE、trace、错误出口或领域语义；runtime focused/full backend test、server build、go list 和 `git diff --check` 均通过。
- RB2-RB4：已完成。清理迁移后错位的 runtime/domain/application/adapter 测试归属，补回窄 adapter/schema/application 合同测试；focused、full backend、server build、go list、diff 和禁止依赖审计通过。
- Batch 1：已完成 `docs/architecture.md` 与本文件的事实快照更新；完整改造计划已补齐目标树、文件/文件组处置表、逐批验证、失败回退、事实标记、pre-mortem 和统一执行协议，当前方案见 `docs/ddd-domain-refactor-plan.md`。
...
- Batch 4：已完成。事件桥接、事件 trace 和 final guard 已在同一 `runtime` package 内按职责重组；事件桥接进一步细分为 `event_bridge.go`（ADK 消费）、`event_chart.go`（chart component 投影）和 `event_output.go`（输出标记投影），另保留 `event_trace.go`、`final_guard.go`。未改变函数签名、API、Graph 拓扑、错误出口、SSE 顺序或 trace 字段。
...
- Batch 9C-1：已完成。动态 `limitations`/`reasoning` 内部引用泄露分类为 `projection_mismatch`，无论错误携带未声明引用还是允许引用目录，均降级为 dynamic facts-only；真正的大运绑定等无引用证据的 `method_contract` 仍硬失败。活动 `/api/chat` 已将请求 context 和 SSE 写失败取消传播到路由、模型、工具与会话锁等待；授权环境全量 backend test、server build、race focused test 和真实 SSE 均通过。`make eval-bazi-quality` 2/2 通过：儿童样例为 dynamic facts-only、成人样例为 dynamic model，二者 static/dynamic/final audit 均为 clean，并观察到 `sse_emit` 与 `contract_gate`。
- DDD Batch B：已完成。`backend/internal/runtime/bazi_authority_sources.go` 的纯来源层级策略迁移到 `backend/internal/specialists/bazi/domain/authority_sources.go`，runtime 三个证据调用点改用 `domain.AuthoritySourceSet` / `domain.StageAuthoritySources`，新增 domain 表格测试；未改 Graph、Session、模型、repair、renderer 或 SSE。授权环境 focused/full test、`go list`、server build、gofmt 和 diff 检查通过。真实虚构 `/api/chat` 回放 trace `trc_83741ae75873` 进入 `bazi_calc/yongshen/dayun_analyzer/bazi_liunian/knowledge_search`，SSE 唯一 `done` 保持；知识库无命中触发既有 `ORCHESTRATION_NO_RESULT` 错误出口，未据此判定业务成功。因 `.git/index` 只读，未能使用 `git mv`，已用新增目标文件加删除旧文件的等价迁移完成。
- DDD Batch C0：已完成。`specialists/bazi/presentation.FinalReplyInput` 只含展示所需的计划、事实、已验收静态/动态/全程槽位和短引文；runtime 的 `bazi_final_presentation.go` 是唯一映射 owner，focused/full backend test、server build、gofmt 和 diff 检查通过。
- DDD Batch C1：已完成。`bazi_final_renderer.go` 保留 runtime 薄入口，纯 Markdown renderer 已迁入 `specialists/bazi/presentation`；原始大运事实解析和合同共用文本归一保留在 runtime。未改变 API、Graph 拓扑、16/24 步上限、repair budget、错误出口、SSE 顺序或领域语义。授权 `make eval-smoke`、全量 backend test、server build、`go list` 和 presentation 依赖审计通过；`.git/index` 只读，采用新增目标文件加删除旧文件的等价迁移。
- DDD Batch D0/D1 只读预检：已完成。CodeGraph 确认 `bazi_graph_adapter.go` 同时绑定 `Executor`、完整 `SessionState`、`EventSink`、trace 和 12 个 Graph callback；`baziInternalGraphState` 在 `bazi_graph_adapter.go`、`bazi_graph_loop.go`、`bazi_internal_graph.go` 间高扇出，当前没有已证明闭合的 adapter/Graph 生产簇。D0 字段矩阵确认 `specialists.Request` 曾携带完整 Session、Route、ManagerContext、DomainContext；D1B/D1C 已将普通 runner 收窄为 `SessionView`，Qimen 仍按 Case/盘面隔离。未复制或移动 Graph/adapter 生产文件；已将 DTO 闭包、状态所有权、无反向依赖和 Graph/SSE/trace 不变量写入方案。
- DDD Batch D1A：已完成。runtime 已改为直接消费已有 `specialists/bazi/graph.Result`，保留 runtime 的领域失败到 `RuntimeFailure` 转换和终态 audit 投影；focused runtime/Graph test、授权环境 full backend test、server build、`go list`、旧符号审计和 diff 检查通过。合成且明确不对应真人的回放 trace `trc_7d72af48f598` 确认外层 16 步、Bazi 24 步、repair budget、`hard_error` 错误出口及唯一 `done` 未变；模型合同失败不归因于本批。原始 `runtime-smoke-v1` 敏感样例未在 D1A 后重跑，保留为 UNKNOWN。
- DDD Batch D1B：已完成。`specialists.Request` 只保留 `UserMessage`、`Route`、`Session`；删除因此产生的 `domainContextFor` 孤儿 helper。focused/full test、server build、`go list`、字段零读取审计和合成 SSE/trace 回放通过；Graph/adapter 未移动。
- DDD Batch D1C：已完成。`specialists.Request` 已改为只接收 `SessionView` 和可选 `SaveToolResult` 回写回调；runtime 在 dispatch 边界生成读投影，普通 runner 不再接收完整 `SessionState`，Bazi Graph 仍由 dispatch 先行分流。focused/full test、server build、`go list`、gofmt、diff 检查、禁止依赖审计均通过；明确非真人回放 trace `4a734b56256aceb9c3924e82f274df24` 保留外层 16 步、Bazi 24 步、一次实际 repair、唯一 `text`/`done` 和 `completed` 终态。Graph/adapter 未移动。
- DDD Batch E1：已完成。新增 `specialists/qimen/domain.Chart/Cell` 与纯 `rotating_8` 符号校验；`tools/qimen` 保留 RFC3339/年份校验、`qimen-go` 排盘和原有 map payload。focused/full test、server build、`go list`、domain 禁止依赖审计、gofmt 和 diff 检查通过；明确非真人回放 trace `trc_5ac7a31df6b2` 保留 `qimen`、外层 16 步、`completed`、Case owner/time、`qimen_dunjia`/`prefill`/`contract_gate` 成功以及唯一 `text`/`done`。Qimen application/graph/presentation 和 Ziwei 算法/Graph 等暂不迁移。
- DDD Batch E2：已完成。将 `qimenChartMatchesTurn`、`qimenQuestionTimeParams` 的纯问事合同迁入 `specialists/qimen/application`，runtime 只保留 Prefill 编排和状态写入；focused/full test、server build、`go list`、gofmt、diff 检查和禁止依赖审计通过。明确非真人回放 trace `trc_d594d57e201a` 保留 `qimen_dunjia` 仅接收 `question_time`、`prefill`、`contract_gate=passed`、Case/time、外层 16 步、`completed` 和唯一 `text`/`done`，无 `error`。Qimen graph/adapter/presentation 与 Ziwei 算法/Graph 等暂不迁移。
- DDD Batch E3：已完成。Qimen specialist 的 `GetConfig`、提示词、知识工具白名单和会话注入配置已迁入 `specialists/qimen/adapter`；composition root 与公共工具白名单测试改用新 owner，旧根包配置零残留。focused/full test、server build、`go list`、`go list -deps`、gofmt、diff 检查和禁止依赖审计通过；明确非真人回放 trace `trc_77efa5501e84` 保留 `question_time`、`prefill`、`contract_gate=passed`、Case/time、外层 16 步、`completed` 和唯一 `text`/`done`，无 `error`。Qimen graph/presentation 与 Ziwei 算法/Graph 等暂不迁移。
- DDD Batch E4：已完成。Qimen `QimenResult` prompt projection 已从 runtime `AgentBuilder` 私有方法迁入 `specialists/qimen/application.BuildDataBlock`；runtime 只传入窄 map payload，旧方法零残留，application 只依赖标准库和既有 `contracts`。focused/full test、server build、`go list`、`go list -deps`、gofmt、diff 检查和禁止依赖审计通过；逐字字段顺序、空值、九宫两种 slice、JSON 兜底和禁止重复排盘提示由 application 测试覆盖。非真人回放 trace `trc_e63c6a1a57e4` 保留 Qimen route、Case/time、`qimen_dunjia` 仅 `question_time` 且来源为 `prefill`、`contract_gate=passed`、外层 16 步、`completed`；SSE `component → thinking → tool_call → thinking → tool_call → tool_call → text → component → component → done`，`text=1`、`done=1`、`error=0`。trace prompt input 按既有持久化限制截断，未将截断内容当作逐字证明。Qimen graph/presentation 与 Ziwei 算法/Graph 等暂不迁移。
- DDD Batch E5：已完成。Ziwei specialist 的静态配置从 `specialists/ziwei` 根包迁入 `specialists/ziwei/adapter`；container 与公共工具白名单测试改用新 owner，配置字段、提示词、知识工具白名单顺序和 `InjectSessionContext` 保持不变，旧根包配置零残留。focused adapter/specialists/container test、授权环境全量 backend test、server build、`go list`、adapter `go list -deps`、gofmt、diff 检查和禁止依赖审计通过。无个人资料的合成紫微澄清回放 trace `trc_846f5802984f` 路由为 `ziwei`，SSE 为唯一 `text → component(run-inspection) → done`，无 `error`；trace status 为 `ok`、外层 `max_run_steps=16`、`termination_reason=short_circuit`。该回放在 preflight 因缺资料短路，未进入 Ziwei specialist，配置运行时实际调用证据仍为 `[UNKNOWN]`；不因此扩大本批范围。
- DDD Batch E6：已完成。Ziwei `ZiWeiResult` prompt projection 已从 runtime `AgentBuilder` 私有方法迁入 `specialists/ziwei/application.BuildDataBlock`；runtime 只传入窄 map payload，旧方法零残留，application 只依赖标准库。focused test、授权环境全量 backend test、server build、`go list`、application 依赖审计、gofmt、`git diff --check` 和禁止依赖审计通过；逐字字段顺序、空值、流年/稀疏 JSON 兜底和禁止重复排盘提示由 application 合同测试覆盖。合成且不含个人出生资料的回放 trace `trc_babddfef660d` 路由为 `ziwei`，SSE 为唯一 `text → component → done`，`text=1`、`done=1`、`error=0`，外层 `max_run_steps=16`、`termination_reason=short_circuit`；preflight 因缺资料短路，未进入 specialist 主链，配置运行时实际调用证据为 `[UNKNOWN]`。
- DDD Batch E7：已完成。Qimen 外部排盘工具从 `internal/tools/qimen` 迁入 `specialists/qimen/adapter`；保留 `tools.Tool` 隐式合同、`question_time` 唯一参数、RFC3339/年份错误、`rotating_8` 符号校验、旧 map payload 和 container 注册。`.git/index` 只读，采用复制目标、改 package/调用者、删除旧 owner 的等价迁移；focused/runtime test、授权环境 `go test ./backend/... -count=1`、server build、`go list`/依赖审计、gofmt、diff 检查和旧路径/旧符号审计通过。合成 Qimen 回放 trace `trc_ffd892d9b9bd` 路由为 `qimen`，`qimen_dunjia` 仅 `question_time`、`prefill`、`contract_gate=passed`、Case/time、外层 16 步、`completed`；SSE `component=3`、`thinking=2`、`tool_call=3`、`text=1`、`done=1`、`error=0`。adapter 包完整依赖闭包经公共 `specialists -> policy -> state` 间接包含 `internal/state`，但 Qimen 工具/domain 无直接 state import，未发现 import cycle；该项留作后续公共契约审查，不扩大 E7。
- DDD Batch E8：已完成。紫微 12 个生产文件和 2 个测试文件从 `internal/tools/ziwei` 迁入 `specialists/ziwei/adapter`；保留 `ziwei_calc`/`ziwei_liunian`、参数校验、真太阳时版本/跨日、四柱/十二宫/流年字段、旧 map payload 和容器注册。`.git/index` 只读，采用完整复制、改 package/调用者、删除旧 owner 的等价迁移；新 adapter/container/runtime focused test、授权环境全量 backend test、server build、`go list`、依赖审计、gofmt、diff 检查和旧路径审计通过。无个人资料澄清回放 trace `trc_eca0c7aa18e1` 路由为 `ziwei`、`status=ok`、`preflight.short_circuit=true`，SSE `text=1`、`component=2`、`done=1`、`error=0`；安全门禁拒绝出生资料载荷，真实 Ziwei specialist 工具调用仍为 `[UNKNOWN]`。
- DDD Batch E9：已完成。将 `constants.go`、`utils.go`、`major.go` 和 `ZiWeiStar` 值对象从 Ziwei adapter 下沉到 `specialists/ziwei/domain`；domain 仅依赖标准库，adapter 保留 `lunar-go` 历法绑定、完整命盘/流年组装、工具入口和 `ToMap`，同名兼容函数只做 domain 委托。domain/adapter/container/runtime focused test、授权环境 `go test ./backend/... -count=1`、server build、`go list`、domain 依赖审计、gofmt、diff 检查和旧 owner/重复实现审计通过；未改 API、Graph 16/24 步、repair、错误出口、SSE 顺序、唯一 `text`/`done` 或领域语义。新增无个人资料澄清 trace `trc_4b7b30c408ed` 为 `status=ok`、`preflight.short_circuit=true`、外层 16 步、`short_circuit` 终态、唯一 `text`/`done`、无 `error`；真实 Ziwei specialist 工具调用和 Prefill/Case 绑定证据仍为 `[UNKNOWN]`。
- DDD Batch E14：已完成。`domain.ZiWeiStar` 已移除 JSON 标签，adapter 的 `StarPayload`/`ToMap` 成为 `name`、`type`、`brightness,omitempty`、`mutagen,omitempty` 的唯一 JSON owner；排盘继续复用同一领域星曜值对象，未复制主/辅/杂星算法。新增 JSON 合同测试固定字段名和空字段省略；focused/full backend test、server build、`go list`、domain 依赖/禁止 JSON 标签/单一 DTO owner、gofmt、diff 审计均通过。最终二进制的无资料 Ziwei trace `trc_f9af1afbaa03` 为 `status=ok`、`preflight.short_circuit=true`、SSE 唯一 `text → component → component → done`、无 `error`；真实 specialist 工具 Prefill/Case 绑定仍为 `[UNKNOWN]`，未将短路回放冒充工具 payload 证据。
- DDD Batch E15：已完成。月系/日系杂曜的无标签索引结果和纯计算已迁入 `specialists/ziwei/domain/monthly_daily_indices.go`；adapter 只保留 `lunar-go` 的农历日、闰月、晚子时月序提取和旧签名转发。focused/full backend test、server build、`go list`、domain 实际 import/单一 owner、gofmt、diff 审计均通过。最终二进制的无资料 Ziwei trace `trc_ee327b7d78cb` 为 `status=ok`、`preflight.short_circuit=true`、SSE 唯一 `text → component → component → done`、无 `error`；真实 specialist 工具 Prefill/Case 绑定仍为 `[UNKNOWN]`，未将短路回放冒充工具输出证据。
- DDD Batch E16：已完成。十四辅星/煞星的无标签组装已迁入 `specialists/ziwei/domain/minor.go`；adapter 保留旧签名转发。domain fixture 锁定 14 星与丙年辅星仅文昌化科；focused/full backend test、server build、`go list`、domain 实际 import/单一 owner、gofmt、diff 审计均通过。最终二进制的无资料 Ziwei trace `trc_e4707c07560f` 为 `status=ok`、`preflight.short_circuit=true`、SSE 唯一 `text → component → component → done`、无 `error`；真实 specialist 工具 Prefill/Case 绑定仍为 `[UNKNOWN]`，未将短路回放冒充工具输出证据。
- DDD Batch E17：已完成。40 颗年/月/日/时杂曜的无标签组装已迁入 `specialists/ziwei/domain/adjective.go`；adapter 保留旧签名转发。domain fixture 精确锁定每宫星曜名称和追加顺序，以及关键类别；focused/full backend test、server build、`go list`、domain 实际 import/单一 owner、gofmt、diff 审计均通过。最终二进制的无资料 Ziwei trace `trc_cb7003eb1912` 为 `status=ok`、`preflight.short_circuit=true`、SSE 唯一 `text → component → component → done`、无 `error`；真实 specialist 工具 Prefill/Case 绑定仍为 `[UNKNOWN]`，未将短路回放冒充工具输出证据。
- DDD Batch E18：已完成。只将命宫/身宫及命宫干支的纯索引计算下沉到 `specialists/ziwei/domain/palace_rules.go`；adapter 保留 lunar-go、四柱、闰月/晚子时归一和兼容签名委托。未修改 runtime、runner、Graph、Session、SSE、trace、API、repair budget 或领域输出语义。focused/full test、server build、`go list`、domain/adapter 依赖与禁止依赖审计、gofmt 和 diff 检查通过；无出生资料回放 trace `trc_25f24af1692d` 为 `status=ok`、`preflight.short_circuit=true`、外层 16 步，SSE 唯一 `text → component(route-decision) → component(run-inspection) → done`，无 `error`。真实 specialist Prefill/Case 绑定仍为 `[UNKNOWN]`。
- Runtime R0：已完成。CodeGraph/源码逐文件盘点 runtime 八字文件，并区分 `specialists/runner.go`、`runtime/specialist_runner.go`、`runtime/execution_dispatch.go`、`tools/runner.go` 的不同 actor 和依赖。确认 `bazi_assertion_contract.go`、`bazi_charter_types.go`、`bazi_graph_*`、`bazi_model_runtime.go` 等高扇出或依赖 runtime 私有 DTO/Graph/Session/trace/repair 的文件不能机械 `cp`。
- Runtime R1：已完成两个已证明闭合的纯领域簇：runtime 的文本归一实现已删除并由 `specialists/bazi/domain/text_list.go` 替代；runtime 的大运事实实现已删除并由 `specialists/bazi/domain/dayun_facts.go` 替代，`baziDayunJudgment` 为保持 JSON 合同的类型别名。两批均通过 domain/runtime focused test，domain 无 runtime 反向依赖。
- Runtime R0.5：已完成。仅将 `backend/internal/runtime/bazi_calendar_rule.go` 重命名为 `backend/internal/runtime/artifact_calendar_rules.go`，同步职责注释和四个函数注释；package、函数、调用方、ExecutionPlan、Prefill、Graph、Session、SSE、trace、API、repair 和领域输出不变。CodeGraph 确认 `calendarRuleForArtifact` 单一调用方，旧路径无残留；这不是 Bazi adapter 迁移，不进入 `bazi_assertion_contract.go` 等下一批。focused/runtime-container-specialist tests、授权全量 backend test、`go build -buildvcs=false`、`go list -buildvcs=false`、依赖/禁止依赖审计和 `git diff --check` 通过；无出生资料回放 trace `trc_1ff21a2c6f71` 为 `status=ok`、`preflight.short_circuit=true`，SSE 唯一 `text → component → component → done`，无 `error`。默认 build/list 的 VCS stamping 权限问题用 `-buildvcs=false` 重跑通过。
...
- 八字 Graph 运行职责已按边界拆分：`bazi_graph_entry.go` 负责内图选择和领域失败归一，`bazi_charter_graph.go` 负责补证、审计、阶段事件和最终 writer 适配，`bazi_contract_validation.go` 负责静态/动态合同，`bazi_final_contract.go` 负责最终文本合同，`bazi_model_runtime.go` 负责确定性分析范围、提示构建和内层 agent 适配；证据规划、受控检索、引用归并和有限补证仍由 `bazi_evidence_runtime.go` 承载，函数签名和 Graph 拓扑不变。
...
- DDD Batch D1C 确认：`specialists/runner.go` 的 Request 仅保留 `policy.ApprovedRoute`、用户消息、`SessionView` 和可选工具结果回写；`SessionView` 只包含 prompt/会话消息所需资料、三类盘面、Subject、最近消息和摘要，不暴露完整 `SessionState`、资产集合或 Manager/Domain context。
- Batch 9A 观察：`runtime` 直接依赖 mcp、tools、tracing，属于跨层观察项；当前尚未证明存在行为安全的窄 DTO，不移动、不新增 adapter。
- DDD Batch D0 确认：`cp` 后改包可以作为 `.git/index` 只读时对已证明闭合文件簇的 `git mv` 等价物，但不能解决 runtime 私有状态、完整 Session、Executor、EventSink、trace 和 Graph callback 的耦合；在 D1 DTO 闭包证明前不复制 Graph/adapter。
- DDD Batch D1/D1C 字段审计确认：`execution_dispatch.go` 是 Request 的唯一生产构造点；`ADKSpecialistRunner.Run` 只依赖 `UserMessage`、`Route`、`SessionView` 和可选回写回调来构建 agent、会话消息、session values 和工具回写；`ManagerContext`/`DomainContext` 仍属于 state/Manager 持久化合同，不跨 specialist 边界传递。
- DDD Batch D1A 确认：合成回放 trace `trc_7d72af48f598` 的 `orchestration.max_run_steps=16`、`bazi.max_run_steps=24`、`repair.max_attempts=1`、`bazi.next_action=hard_error` 和 `bazi.termination_reason=hard_error` 与既有合同一致；SSE 唯一 `done` 保持。原始 `runtime-smoke-v1` 两条含出生资料样例未重跑，不能用作本批成功样例证据。
...
- Batch 9B/D1C 确认：历史审计时 `specialists.Request` 曾直接携带 Manager/Domain context 和完整 Session；D1C 已改为显式 `SessionView`，并将回写保留为 runtime 单点回调。Qimen 仍通过 `specialistSessionView` 收窄为 Case/盘面，Bazi/Ziwei 共享的也只是 runtime 生成的读投影。
...
- 静态层拥有本命基础结构；完整命盘新增独立 `lifetime_dayun_judgment`，逐步覆盖全部已计算大运并输出全程运路；动态层仍只拥有当前大运、流年走势与承接状态。三层互不改写，最终成文先给出本命总览，再按“强弱/调候/格局视角（含格局评价、古籍参照、断语所限）→ 全程运路 → 当前应期”展开；动态仍不得伪造本命事实或改写全程逐运结论。
...
- renderer 在所有最终文本出口删除内部引用路径；总览结论置于完整报告开头，只收束主轴、层次、可发挥处、限制和发挥方向；全程与当前只在各自详细章节呈现。调候有效性未知时，用例投影强制保留“待确认”边界，不透传“火根不足已制约层次”等未经确认的模型结论。格局、强弱、调候和层次各自只呈现所属裁断；动态 facts-only 只展示已绑定当前大运或明确未定位的可复算事实，不把全量人生大运目录伪装为动态解读。动态两个有效结论均为 facts-only 模板（包括模型缺失后由投影补值）时，用例投影直接采用确定性动态降级对象，避免正文已声明“只展示事实”却仍显示年性、依据或限制。
...
- Qimen E1 的 `specialists/qimen/domain` 仅依赖标准库，typed 盘面和跨体系符号拒绝规则已与 `tools/qimen` 的旧 map payload 解耦；未新增 Qimen Graph 或外部依赖。
- E1 非真人 SSE 回放顺序为 `component → thinking → tool_call → thinking → tool_call → tool_call → text → component → component → done`，`text=1`、`done=1`、`error=0`；trace `trc_5ac7a31df6b2` 为 `qimen`、外层 16 步、`completed`。
- Qimen E2 的 `specialists/qimen/application` 只依赖公共 `contracts.TurnContext` 和标准库；`QuestionTimeParams`、`MatchesStoredCaseChart` 不接触 runtime、Executor、完整 SessionState、工具、模型、检索、trace 或 SSE。
- E2 非真人 SSE 回放顺序为 `component → thinking → tool_call → thinking → tool_call → thinking → tool_call → text → component → component → done`，`text=1`、`done=1`、`error=0`；trace `trc_d594d57e201a` 的 `qimen_dunjia` 参数键为 `question_time`，`prefill.executed=true`，`contract_gate=passed`，外层 16 步、`completed`，Case/time 绑定保持。
- Qimen E3 的 `specialists/qimen/adapter` 配置直接依赖 `prompts` 和公共 `specialists.Config`；Qimen 配置/工具不直接依赖 runtime、SessionState、工具执行、trace 或 SSE，composition root 只负责组装。完整 adapter 依赖闭包经 `specialists -> policy -> state` 间接包含 `internal/state`，留作后续公共契约审查。
- E3 非真人 SSE 回放顺序为 `component → thinking → tool_call → thinking → tool_call → tool_call → thinking → tool_call → text → component → component → done`，`text=1`、`done=1`、`error=0`；trace `trc_77efa5501e84` 保留 Qimen route、`qimen_dunjia` `question_time`、`prefill.executed=true`、`contract_gate=passed`、外层 16 步、`completed` 和 Case/time 绑定。
- Qimen E4 的 `specialists/qimen/application.BuildDataBlock` 只依赖标准库和既有 `contracts`，runtime 不再持有 Qimen prompt projection；application 不接触 runtime、SessionState、Executor、工具、模型、检索、trace 或 SSE。
- E4 非真人 SSE 回放顺序为 `component → thinking → tool_call → thinking → tool_call → tool_call → text → component → component → done`，`text=1`、`done=1`、`error=0`；trace `trc_e63c6a1a57e4` 为 `qimen`、`qimen_dunjia` 参数键仅 `question_time` 且来源为 `prefill`、存在 `prefill`/`contract_gate=passed`、外层 16 步、`completed` 和 Case/time 绑定。
...
- 静态 DTO 已收窄：模型只能输出四项 4-80 字的静态短裁断、状态、事实/规则引用，以及主格、成局路线、子平定性三个字段；不接收原局 relation ID、自由边界、限制或推理文本。runtime 依据 `BaziFactCapsule` 投影这些边界，避免未声明 `relation.natal.*` 耗尽唯一修复机会。
...
- 1991 命例已在本轮真实 SSE 返回完整章节且无 `error`；官星透藏仍由 runtime 的确定性事实投影，不再要求静态模型输出独立风险状态。
...
- DDD Batch E14 已完成：领域星曜是无标签值对象，工具输出 JSON DTO 固定在 adapter `ToMap`；focused/full test、server build、依赖/单一 owner 审计和无资料 SSE/trace 回放均通过。最终二进制 trace `trc_f9af1afbaa03` 为 `status=ok`、`preflight.short_circuit=true`、唯一 `text → component → component → done`、无 `error`；真实 specialist 工具 Prefill/Case 绑定保持 `[UNKNOWN]`。
- DDD Batch E15 已完成：月/日杂曜索引是无标签 domain 值对象，adapter 只保留 lunar-go 月日归一与旧签名；focused/full test、server build、依赖/单一 owner 审计和无资料 SSE/trace 回放均通过。最终二进制 trace `trc_ee327b7d78cb` 为 `status=ok`、`preflight.short_circuit=true`、唯一 `text → component → component → done`、无 `error`；真实 specialist 工具 Prefill/Case 绑定保持 `[UNKNOWN]`。
- DDD Batch E16 已完成：十四辅星/煞星组装是无标签 domain 规则，adapter 只保留旧签名转发；focused/full test、server build、依赖/单一 owner 审计和无资料 SSE/trace 回放均通过。最终二进制 trace `trc_e4707c07560f` 为 `status=ok`、`preflight.short_circuit=true`、唯一 `text → component → component → done`、无 `error`；真实 specialist 工具 Prefill/Case 绑定保持 `[UNKNOWN]`。
- DDD Batch E17 已完成：年/月/日/时杂曜组装是无标签 domain 规则，adapter 只保留旧签名转发；focused/full test、server build、依赖/单一 owner 审计和无资料 SSE/trace 回放均通过。最终二进制 trace `trc_cb7003eb1912` 为 `status=ok`、`preflight.short_circuit=true`、唯一 `text → component → component → done`、无 `error`；真实 specialist 工具 Prefill/Case 绑定保持 `[UNKNOWN]`。
- DDD/Clean Architecture Batch D1A、D1B、D1C、Qimen E1-E7、Ziwei E5-E18 已完成；D1C 合成回放 trace 为 `4a734b56256aceb9c3924e82f274df24`，E1-E4 Qimen 回放 trace 为 `trc_5ac7a31df6b2`、`trc_d594d57e201a`、`trc_77efa5501e84`、`trc_e63c6a1a57e4`，E5 Ziwei 入口回放 trace 为 `trc_846f5802984f`，E6 Ziwei 澄清回放为 `trc_babddfef660d`，E7 Qimen 工具回放为 `trc_ffd892d9b9bd`，E8 Ziwei 澄清回放边界为 `trc_eca0c7aa18e1`，E9 Ziwei 澄清回放为 `trc_4b7b30c408ed`，E10 Ziwei 澄清回放为 `trc_c1ac116bf1b6`，E11 为 `trc_3a15c0307917`，E12 为 `trc_04aeb2b2680a`，E13 为 `trc_41e0da5b1c21`，E14 为 `trc_f9af1afbaa03`，E15 为 `trc_ee327b7d78cb`，E16 为 `trc_e4707c07560f`，E17 为 `trc_cb7003eb1912`，E18 为 `trc_25f24af1692d`；真实含出生资料的 `runtime-smoke-v1` 因安全门禁未执行。E18 后不继续紫微迁移；Qimen graph/presentation 与 Ziwei 更深历法 domain、graph/presentation deferred。Runtime R0/R0.5 已完成；R1 仅在窄 DTO 闭包证明后执行。
- Batch 9C-1 行为基线已完成；确定性测试、授权全量 backend test、server build 和 `runtime-smoke-v1` 2/2 已通过。动态合同波动与活动 cancel 仍是已记录风险，不因 E18 自动进入 package 拆分。Runtime R0/R0.5/R1 及 Runtime Clean Governance G0-G4 已完成；当前不进入 G5 package 拆分。
- runtime 清理已完成 R0.5 的最小同包重命名和 R1 的两个纯领域簇迁移。下一批是 assertion/校验/综合的高扇出合同簇，需先证明窄 DTO；不得机械移动。`bazi_graph_*`、模型/检索 adapter、Session/事件桥继续留在 runtime，直至端口闭合。
- 高可用另立 H0-H3 专项；当前 `MemoryStore`、本地 JSON 和 `MemoryLocker` 不作为多实例高可用证明，未确定部署基础设施前不改 provider。
- 结构化输出合同：V2 调用 BaZi `analysis_plan`、`evidence_plan`、`static_synthesis`、`dynamic_synthesis`、`lifetime_dayun_synthesis` 五类 JSON Mode DTO。每份 Schema 通过 go:embed 进入同一个 registry；prompt 注入、原始 JSON 校验和 hash 都消费对应文件原文，Go DTO 只负责严格解码后的语义校验。链路为 registry -> prompt -> gojsonschema -> DisallowUnknownFields + EOF -> DTO/引用 catalog；catalog 向模型提供 `{id, hint}` 供选择，输出仍只允许 ID。`evidence_plan` 生成最多两条受约束查询，初检无可用原文或高冲突时最多补搜一条；它不驱动 facts-only，命中只作为同轮综合节点的可选材料。静态节点的四条短裁断必须以具名 `slot` 唯一标识，领域层按名称归一而不依赖模型数组顺序；边界与限制由 runtime 投影。动态节点才允许岁运关系引用。空输出、fence、缺字段、错类型、未知字段与 trailing JSON 均拒绝。未知引用允许一次本节点定向 repair；事实值冲突和方法合同冲突仍硬失败。ADK output tool 维持 InferTool/ReturnDirectly 与既有 optional/Normalize 语义；Manager、ExecutionPlan、Prefill、outer graph、Qimen/Ziwei 自由文本与 SSE wire shape 均未改。json_object 不是 provider-native strict schema；动态未授权范围按既定 facts-only policy 降级。2026-08-15 真实回放 `trc_93e36e676e37` 已验证静态具名槽位、`0/3` 检索预算和唯一 `text -> done`。
...
- 真实回放已验证澄清路径的 `Graph Invoke -> final guard -> 唯一 text -> done` 收口；授权 `runtime-smoke-v1` 2/2 也通过并记录 `sse_emit`、`contract_gate`。本轮 G3/G4 未启动新服务，未新增真实 `/api/chat` 回放；当前完整八字主链仍出现动态合同 hard error，活动 cancel 仍缺本轮在线证据。这些是既有风险，不归因于 E18，也不授权进入 package 拆分或继续紫微迁移。
...
- 八字：runtime 适配与高扇出领域节点仍在 `backend/internal/runtime/bazi_internal_graph.go`、`bazi_graph_loop.go`、`bazi_graph_entry.go`、`bazi_charter_graph.go`、`bazi_contract_validation.go`、`bazi_final_contract.go`、`bazi_model_runtime.go`、`bazi_canonical_synthesis.go`、`bazi_final_presentation.go`、`bazi_final_renderer.go`；纯 Markdown renderer 在 `backend/internal/specialists/bazi/presentation/`，事实胶囊、年龄授权、引用目录、文本归一和大运事实合同在 `backend/internal/specialists/bazi/domain/`；确定性排盘在 `backend/internal/tools/bazi/`。
- 奇门：纯盘面合同在 `backend/internal/specialists/qimen/domain/`，问事合同和 prompt projection 在 `backend/internal/specialists/qimen/application/`，排盘、旧 map payload 和配置在 `backend/internal/specialists/qimen/adapter/`；旧 `backend/internal/tools/qimen/` 已删除。
- 紫微：纯星曜常量、纯年/时定位规则、年系/时系索引结果、索引/时辰/亮度/四化辅助、无标签 `ZiWeiStar` 和主星排布在 `backend/internal/specialists/ziwei/domain/`；`StarPayload`、specialist 配置、`lunar-go` 绑定的起紫微/宫位/月日计算、流年工具和旧 map payload 在 `backend/internal/specialists/ziwei/adapter/`，prompt projection 在 `backend/internal/specialists/ziwei/application/`；剩余历法/graph/presentation 尚未迁移。
