...
- Batch 9C-1 行为基线已完成；确定性测试、授权全量 backend test、server build 和 `runtime-smoke-v1` 2/2 已通过。动态合同波动与活动 cancel 仍是已记录风险，不因 E18 自动进入 package 拆分；当前先做 runtime R0 只读盘点。
...
- 真实回放已验证澄清路径的 `Graph Invoke -> final guard -> 唯一 text -> done` 收口；授权 `runtime-smoke-v1` 2/2 也通过并记录 `sse_emit`、`contract_gate`。当前完整八字主链仍出现动态合同 hard error，活动 cancel 没有真实 `/api/chat` 证据；这些是既有风险，不归因于 E18，也不授权进入 package 拆分或继续紫微迁移。
