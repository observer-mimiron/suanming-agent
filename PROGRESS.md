...
- renderer 在所有最终文本出口删除内部引用路径；总览结论置于完整报告开头，只收束主轴、层次、可发挥处、限制和发挥方向；全程与当前只在各自详细章节呈现。调候有效性未知时，用例投影强制保留“待确认”边界，不透传“火根不足已制约层次”等未经确认的模型结论。格局、强弱、调候和层次各自只呈现所属裁断；动态 facts-only 只展示已绑定当前大运或明确未定位的可复算事实，不把全量人生大运目录伪装为动态解读。
