#!/usr/bin/env node
/**
 * knowledge MCP server — exposes wiki tools over stdio transport.
 *
 * Tools:
 *   search_wiki    — Search wiki pages by query string
 *   retrieve_passages — Retrieve hybrid-ranked passages without answer synthesis
...
import { retrievePassages, type RetrievalPassage } from "./lib/query-search";
...
import { resolveScope, resolveScopeSlugs, type ContentSearchResult } from "./lib/search";
...
export async function handleRetrievePassages(args: {
  query: string;
  limit?: number | undefined;
  scope?: string | undefined;
  ...
  const readable = await listReadableWikiPages(null);
  const entries = scopeSlugs
    ? (() => {
        const allowed = new Set(scopeSlugs);
        return readable.filter((entry) => allowed.has(entry.slug));
      })()
    : readable.filter(
        (entry) => !isAgentScopedType(entry.type) && !isArtifactType(entry.type),
      );

  return withPageCache(() => retrievePassages(args.query, entries, limit));
}
...
const SERVER_INSTRUCTIONS = `KNOWLEDGE_BASE_NAME is a shared wiki for humans and agents. It accumulates durable, citable knowledge — not ephemeral RAG results. Every page has confidence scores, expiry dates, cited sources, and revision history. Contradictions are tracked and resolved through talk pages.
...
export function createMcpServer(): McpServer {
  ...
  server.registerTool("search_wiki", {
    description: "Search knowledge wiki pages by query string",
    inputSchema: {
  ...
  // retrieve_passages — Hybrid retrieval without answer synthesis
  server.registerTool("retrieve_passages", {
    description:
      "Retrieve hybrid-ranked wiki passages for a query using BM25 plus vector similarity fusion, without synthesizing a final answer",
    inputSchema: {
      query: z.string().describe("Search query"),
      limit: z
        .number()
        .optional()
        .describe("Maximum number of passages to return (default 10)"),
      scope: z
        .string()
        .optional()
        .describe("Scope retrieval to an agent's pages, e.g. 'agent:yoyo'"),
    },
    annotations: {
      readOnlyHint: true,
      destructiveHint: false,
    ...
    try {
      const results = await handleRetrievePassages(args);
      return {
        content: [
          {
            type: "text" as const,
  ...
  server.registerTool("read_page", {
    description: "Read a single knowledge wiki page by slug",
    inputSchema: {
  ...
  server.registerTool("list_pages", {
    description:
      "List all knowledge wiki pages with optional sort and limit",
    inputSchema: {
      sort: z
  ...
  server.registerTool("create_page", {
    description: "Create a new knowledge wiki page with the given slug and markdown content",
    inputSchema: {
  ...
  server.registerTool("update_page", {
    description: "Update an existing knowledge wiki page with new markdown content",
    inputSchema: {
  ...
  server.registerTool("delete_page", {
    description: "Delete a knowledge wiki page by slug",
    inputSchema: {
  ...
  server.registerTool("lint_wiki", {
    description:
      "Run quality checks on the knowledge wiki. Returns an array of issues with type, severity, slug, and message. Optionally scope to specific check types or minimum severity.",
    inputSchema: {
      checks: z
  ...
  server.registerTool("dataview_query", {
    description:
      "Query knowledge wiki pages by frontmatter fields with structured filters, sort, and limit. " +
  ...
  server.registerTool("list_revisions", {
    description:
      "List revision history for a knowledge wiki page. Returns metadata for each revision " +
  ...
  server.registerTool("read_revision", {
    description:
      "Read the full content of a specific revision of a knowledge wiki page. " +
