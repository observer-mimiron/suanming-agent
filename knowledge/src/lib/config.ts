 ...
 *
 * True when:
 *   1. `KNOWLEDGE_READONLY=1` environment variable is set, **or**
...
export function isReadOnly(): boolean {
  if (process.env.KNOWLEDGE_READONLY === "1") return true;
...
/** Returns embedding-specific base URL, falling back to OPENAI_BASE_URL. */
export function getEmbeddingBaseUrl(): string | undefined {
  const cfg = loadConfigSync();
  return process.env.EMBEDDING_BASE_URL ?? cfg.embeddingBaseUrl ?? process.env.OPENAI_BASE_URL ?? undefined;
}

/** Returns the effective OpenAI-compatible base URL. */
