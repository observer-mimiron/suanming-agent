...
function reqWith(authHeader?: string): Request {
  ...
  return new Request("https://knowledge.example/api/agents/seed", {
    method: "POST",
    headers,
  });
}
...
beforeEach(() => {
  savedToken = process.env.KNOWLEDGE_SERVICE_TOKEN;
  savedHandle = process.env.KNOWLEDGE_SERVICE_PRINCIPAL;
  process.env.KNOWLEDGE_SERVICE_TOKEN = TOKEN;
  process.env.KNOWLEDGE_SERVICE_PRINCIPAL = HANDLE;
});

afterEach(() => {
  if (savedToken === undefined) delete process.env.KNOWLEDGE_SERVICE_TOKEN;
  else process.env.KNOWLEDGE_SERVICE_TOKEN = savedToken;
  if (savedHandle === undefined) delete process.env.KNOWLEDGE_SERVICE_PRINCIPAL;
  else process.env.KNOWLEDGE_SERVICE_PRINCIPAL = savedHandle;
});
