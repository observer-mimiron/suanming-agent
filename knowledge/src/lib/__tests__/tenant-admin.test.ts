...
describe("deleteTenant", () => {
  it("deletes only the tenant's OWNED pages (flat + silo) and its folder", async () => {
    ...
    await write("seed", {}); // ownerless → knowledge
  ...
  it("ownerless/seed pages belong to the knowledge tenant", async () => {
    await write("seed-a", {});
    await write("seed-b", { owner: "knowledge" });
    await write("alice-x", { owner: "alice" });

    const result = await deleteTenant("knowledge");
...
describe("DELETE /api/admin/tenant/[handle] — gating", () => {
  ...
  it("400 when authed but confirmation is missing/wrong", async () => {
    process.env.KNOWLEDGE_SERVICE_TOKEN = "tok";
    process.env.KNOWLEDGE_SERVICE_PRINCIPAL = "knowledge";
  ...
  it("admins (service token) CAN delete the platform knowledge tenant", async () => {
    process.env.KNOWLEDGE_SERVICE_TOKEN = "tok";
    process.env.KNOWLEDGE_SERVICE_PRINCIPAL = "knowledge";
    await write("seed", {}); // ownerless → knowledge

    const res = await del("knowledge", { token: "tok", confirm: "knowledge" });
    expect(res.status).toBe(200);
    expect((await res.json()).deletedPages).toBe(1);
