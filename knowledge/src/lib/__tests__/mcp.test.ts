...
// ---------------------------------------------------------------------------
// retrieve_passages tests
// ---------------------------------------------------------------------------

describe("retrieve_passages", () => {
  it("returns hybrid retrieval passages for matching content", async () => {
    await writeTestPage(
      "ziping-geju",
      "# 子平真诠 论格局\n\n格局以月令为主，先观提纲，再察全局。",
    );
    await writeTestPage(
      "sanming-dayun",
      "# 三命通会 论大运\n\n大运主十年之气，须结合原局强弱与岁运并看。",
    );
    await writeIndex([
      { title: "子平真诠 论格局", slug: "ziping-geju", summary: "格局以月令为主" },
      { title: "三命通会 论大运", slug: "sanming-dayun", summary: "大运主十年之气" },
    ]);

    const results = await handleRetrievePassages({ query: "格局 月令", limit: 2 });
    expect(results.length).toBeGreaterThanOrEqual(1);
    expect(results[0].slug).toBe("ziping-geju");
    expect(results[0].title).toBe("子平真诠 论格局");
    expect(results[0].snippet).toContain("格局");
    expect(typeof results[0].score).toBe("number");
  });

  it("returns empty array when retrieval finds no matches", async () => {
    await writeTestPage(
      "ziping-geju",
      "# 子平真诠 论格局\n\n格局以月令为主。",
    );
    ...
    const results = await handleRetrievePassages({ query: "量子纠缠", limit: 3 });
    expect(results).toEqual([]);
  });

  it("respects explicit agent scope", async () => {
    await writeTestPage(
      "agent-page",
      "---\ntitle: Agent Page\ntype: agent-knowledge\n---\n# Agent Page\n\n只在 agent scope 下可见的知识。",
    );
    await writeIndex([
      { title: "Agent Page", slug: "agent-page", summary: "只在 agent scope 下可见的知识" },
    ]);
    await registerAgent({
      id: "test-agent",
      name: "Test Agent",
      description: "Test",
      identityPages: [],
      learningPages: ["agent-page"],
      socialPages: [],
      registered: new Date().toISOString(),
      lastUpdated: new Date().toISOString(),
    });

    const scoped = await handleRetrievePassages({
      query: "可见 知识",
      scope: "agent:test-agent",
      limit: 5,
    });
    expect(scoped.map((r) => r.slug)).toContain("agent-page");

    const unscoped = await handleRetrievePassages({
      query: "可见 知识",
      limit: 5,
    });
    expect(unscoped.map((r) => r.slug)).not.toContain("agent-page");
  });
});
...
describe("MCP write tools", () => {
  describe("create_page", () => {
    ...
    it("includes all knowledge schema fields in frontmatter", async () => {
