...
describe("ownerToTenant", () => {
  ...
  it("falls back to DEFAULT_TENANT (knowledge) when ownerless", () => {
    expect(DEFAULT_TENANT).toBe("knowledge");
    expect(ownerToTenant(undefined)).toBe("knowledge");
    expect(ownerToTenant(null)).toBe("knowledge");
    expect(ownerToTenant("")).toBe("knowledge");
    expect(ownerToTenant("   ")).toBe("knowledge");
  });
  ...
  it("normalizes path-unsafe chars so the tenant is always a valid URL/folder", () => {
    ...
    expect(ownerToTenant("..")).toBe("knowledge"); // ".." → trimmed empty → default
  });
...
describe("resolveSlugPath", () => {
  ...
  it("falls back to the linking page's tenant for unknown (dangling) targets", () => {
    ...
    expect(resolveSlugPath("missing", undefined, "knowledge")).toBe(
      "/u/knowledge/missing",
    );
  });
  ...
  it("keeps non-commons targets owner-scoped even with a commons set provided", () => {
    ...
    expect(resolveSlugPath("ghost", map, "knowledge", new Set(["ghost"]))).toBe(
      "/wiki/ghost",
    );
  });
