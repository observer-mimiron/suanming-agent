...
describe("syncOwnerIndexForPage (after seeding)", () => {
  ...
  it("ownerless pages land in the default (knowledge) tenant", async () => {
    await syncOwnerIndexForPage("o");
    expect((await getOwnerIndex())?.knowledge).toEqual(["o"]);
  });
