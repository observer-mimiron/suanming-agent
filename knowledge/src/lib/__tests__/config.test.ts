...
describe("isReadOnly", () => {
  it("returns false by default", () => {
    delete process.env.KNOWLEDGE_READONLY;
    delete process.env.STORAGE_PROVIDER;
    expect(isReadOnly()).toBe(false);
  });

  it("returns true when KNOWLEDGE_READONLY=1", () => {
    process.env.KNOWLEDGE_READONLY = "1";
    expect(isReadOnly()).toBe(true);
  });

  it("returns false when KNOWLEDGE_READONLY is set to something other than 1", () => {
    process.env.KNOWLEDGE_READONLY = "0";
    expect(isReadOnly()).toBe(false);
  });
...
describe("getEffectiveSettings readOnly", () => {
  ...
  it("includes readOnly: true when KNOWLEDGE_READONLY=1", () => {
    process.env.KNOWLEDGE_READONLY = "1";
    const s = getEffectiveSettings();
