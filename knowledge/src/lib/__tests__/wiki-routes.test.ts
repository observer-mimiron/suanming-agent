...
describe("POST /api/wiki — knowledge metadata", () => {
  ...
  it("sets default knowledge frontmatter on new page", async () => {
...
describe("PUT /api/wiki/[slug] — contributors and updated", () => {
  ...
  /** Create a page with full knowledge metadata so PUT has something to edit. */
...
describe("PATCH /api/wiki/[slug] — metadata updates", () => {
  ...
  /** Create a page with full knowledge metadata so PATCH has something to edit. */
  async function seedPage(slug: string, fm: Frontmatter = {}) {
    const today = new Date().toISOString().slice(0, 10);
    const defaults: Frontmatter = {
      created: today,
