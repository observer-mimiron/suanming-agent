...
// ---------------------------------------------------------------------------
// Unmigrated-page check — flags pages missing ALL core knowledge metadata.
// ---------------------------------------------------------------------------

/**
 * Check for unmigrated pages — pages ingested before Phase 1 that lack ALL
 * three core knowledge fields (confidence, authors, expiry). A page that has
  ...
  for (const entry of pages) {
    ...
    if (!hasConfidence && !hasAuthors && !hasExpiry) {
      issues.push({
        ...
        slug: entry.slug,
        message: `Page lacks knowledge metadata — run auto-fix to migrate`,
        severity: "info",
