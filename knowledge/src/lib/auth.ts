...
// and it resolves to ONE fixed principal handle (KNOWLEDGE_SERVICE_PRINCIPAL) so
// everything it writes is attributed and owner-gated like any other user.
//
// Set both as Worker secrets:
//   wrangler secret put KNOWLEDGE_SERVICE_TOKEN       (a long random string)
//   wrangler secret put KNOWLEDGE_SERVICE_PRINCIPAL   (the handle it acts as)
 ...
 *
 * Returns a principal ONLY when both `KNOWLEDGE_SERVICE_TOKEN` and
 * `KNOWLEDGE_SERVICE_PRINCIPAL` are configured and the request presents the
...
export function getServicePrincipal(req: Request): Principal | null {
  const expected = process.env.KNOWLEDGE_SERVICE_TOKEN;
  const handle = process.env.KNOWLEDGE_SERVICE_PRINCIPAL;
