...
// ---------------------------------------------------------------------------
// Unmigrated-page fix — adds sensible knowledge defaults to pre-Phase-1 pages.
// ---------------------------------------------------------------------------

/**
 * Fix an unmigrated-page lint issue by adding sensible knowledge defaults.
 *
...
export async function fixUnmigratedPage(slug: string): Promise<FixResult> {
  ...
  await writeWikiPageWithSideEffects({
    ...
    logDetails: () =>
      added.length > 0
        ? `auto-fix: added knowledge defaults: ${added.join(", ")}`
        : `auto-fix: unmigrated page already has all knowledge fields`,
    crossRefSource: null,
    author: "lint-fix",
  });
  ...
  return {
    ...
    message: added.length > 0
      ? `Added knowledge defaults: ${added.join(", ")}`
      : `Page already has all knowledge fields — no changes needed`,
  };
}
