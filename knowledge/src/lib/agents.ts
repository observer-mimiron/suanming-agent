// ---------------------------------------------------------------------------
// Agent registry — Phase 4 agent identity as knowledge pages (data layer)
// ---------------------------------------------------------------------------
//
...
//   id = slugify("<owner>-<name>")   e.g. "knowledge-yoyo", "alice-yoyo"
...
export const BASE_AGENT_OWNER = "knowledge";
...
export async function seedAgent(options: SeedAgentOptions): Promise<AgentProfile> {
  ...
  // Composite id so each owner can have their own "<name>" (e.g. "knowledge-yoyo").
...
export interface ForkAgentOptions {
  ...
  owner: string;
  /** The id of the template to fork from (e.g. the base "knowledge-yoyo"). */
  templateId: string;
